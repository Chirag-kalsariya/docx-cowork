...
> **Not supported:** presentations (`.ppt`, `.pptx`, `.key`), spreadsheets
> (`.xlsx`, `.numbers`), web pages (`.html`), or markup files (`.md`, `.rst`).
> Politely decline these and tell the user what IS supported.
...
1. **Always clean up.** Delete every temporary file when you are done,
   whether the task succeeded or failed. Temp dirs are prefixed `docx_cowork_`.
2. **Check dependencies first.** Confirm `pandoc` (and `LibreOffice` when
   needed) are available before starting any conversion.
3. **Unsupported format?** Tell the user clearly and list what IS supported.
4. **Google Docs?** Ask the user to export to `.docx` first
   (File → Download → Microsoft Word `.docx`), then proceed.
